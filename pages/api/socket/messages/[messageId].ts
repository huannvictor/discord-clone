...
export default async function handler(
  ...
  try {
    ...
    if (!server) return res.status(400).json({ error: 'Server not fund' })
    ...
    if (!channel) return res.status(400).json({ error: 'Channel not fund' })
    ...
    if (!message || message.deleted) return res.status(400).json({ error: 'Message not fund' })
